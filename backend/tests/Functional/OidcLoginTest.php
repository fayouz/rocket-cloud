...
final class OidcLoginTest extends WebTestCase
{
    ...
    public function testProvidersAreListedPublicly(): void
    {
        ...
        self::assertSame('Rocket Cloud', $response['providers'][0]['name']);
        self::assertSame('https://auth.example.org/authorize', $response['providers'][0]['authorizationEndpoint']);
        self::assertSame('rocket-cloud', $response['providers'][0]['clientId']);
    ...
    public function testSignInCreatesTheAccountAndOpensASession(): void
    {
        ...
        self::assertSame('Rocket Cloud', $me['user']['authenticationServerName']);
    ...
    public function testAdminsConfigureProvidersWithAnEncryptedSecret(): void
    {
        ...
        $created = $this->api('POST', '/api/authentication_servers', [
            'name' => 'Rocket Cloud', 'type' => 'oidc', 'enabled' => true, 'url' => self::ISSUER, 'internalUrl' => self::INTERNAL.'/',
            'clientId' => 'rocket-cloud', 'clientSecret' => 'top-secret', 'scopes' => 'openid  email profile',
        ], $auth);
    ...
    private function createServer(string $name = 'Rocket Cloud', bool $enabled = true, string $adminGroup = ''): AuthenticationServer
    {
    ...
    {
        $idToken = Jwt::sign($claims + ['iss' => self::ISSUER, 'aud' => 'rocket-cloud', 'iat' => time(), 'exp' => time() + 300], $key ?? $this->key, Jwt::publicJwk($this->key)['kid']);
