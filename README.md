...
- Comptes **locaux** (mot de passe haché), **LDAP** (authentification par bind sur l'annuaire) ou **SSO** : connexion via un fournisseur **OpenID Connect** comme [Rocket Cloud](https://github.com/fayouz/rocket-cloud) (Administration → Serveurs d'authentification). Voir `docs/content/5.administration/8.sso.md`.
...
Un administrateur crée une application. Son jeton secret (`rca_…`) n'est affiché qu'une seule fois, et seul son hash SHA-256 est stocké.

| En-têtes | Effet |
|---|---|
| `Authorization: Bearer rca_…` | L'application s'identifie (accès limité à `GET /api/me`). |
...
1. **Côté serveur de l'application tierce** (le secret ne doit jamais aller dans le navigateur) :
   ...
   curl -X POST https://mailer.exemple.com/api/embed/token \
     -H "Authorization: Bearer rca_…" -H "X-Impersonate-User: jean@exemple.org"
...
2. **Côté navigateur** :
   ...
   <script>
     RocketMailer.mount('#mailer', {
       ...
       getToken: () => fetch('/mon-backend/rocket-cloud-token').then(r => r.json()).then(d => d.token),
...
```html
<rocket-cloud-composer application-id="<uuid>" token-url="/mon-backend/rocket-cloud-token"></rocket-cloud-composer>
```
...
- sur `main`, `develop` et les tags `v*` : build et push des images sur **ghcr.io** :
  - `ghcr.io/fayouz/rocket-cloud-api`
  - `ghcr.io/fayouz/rocket-cloud-front`
