...
export function useAppVersion() {
  ...
  const auth = useAuth()
  const api = useApi()
  const apiVersion = useState<AppVersionInfo | null>('rc_api_version', () => null)
  const update = useState<UpdateStatus | null>('rc_update_status', () => null)
  const loaded = useState('rc_version_loaded', () => false)
