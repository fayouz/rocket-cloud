...
## Repères
- `app_id` `cloud`, jetons d'application `rca_…`, ports front 3200 · api 8200 · docs 3201.
- Domaine : fichiers et partage : `src/Files`, `src/Share` (liens publics `/s/<token>`, notifications via Rocket Mailer : `MailerClient`, jeton Rocket Auth en suite, `ROCKET_MAILER_TOKEN` sinon).
...
- Pages publiques : `publicPaths: ['/s/']` dans `app.config.ts`.
